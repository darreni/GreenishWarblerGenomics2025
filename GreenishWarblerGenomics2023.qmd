...
#### specific scaffolds for windowed Fst
```{julia}
scaffolds_for_Fst = vec(["gw2",
                            "gw1",
                            "gw3",
                            "gwZ",
                            "gw1A",
...
function rollingMean(inputVector, windowSize)
...
function getWindowedFst(FstNumerator, FstDenominator, pos, windowSize)
    rollingMeanPos = rollingMean(pos.position, windowSize) # get per-window mean SNP position along scaffold
    rollingMeanFst = Array{Float64, 2}(undef, size(FstNumerator, 1), length(rollingMeanPos))
    for i in 1:size(FstNumerator, 1)
        rollingMeanFstNumerator = rollingMean(FstNumerator[i,:], windowSize)
        rollingMeanFstDenominator = rollingMean(FstDenominator[i,:], windowSize)
...
function plotGenomeFst(scaffolds_to_plot, 
    ...
    f = CairoMakie.Figure(backgroundcolor = RGBf(0.5, 0.5, 0.5),resolution=figureSize)
    numRows = length(scaffoldInRow)
    # make one axis object per row:
    axs = [Axis(f[i, 1]) for i in 1:numRows]
    for i in 1:numRows
        hidedecorations!(axs[i]) #hide background lattice and axis labels
        hidespines!(axs[i]) # hide box around plot
    end
    display(f)
    ...
    for rowNum in 1:numRows
        ...
        for orderNum in 1:numScaffoldsInRow 
            ...
            plotBpLength = scaffold_lengths[scaffolds_to_plot .== scaffoldName]
            ...
            #CairoMakie.poly!(axs[1], Point2f.([0, 0, 1, 1], [0, 1, 1,0]), color=:red)         
            for j in 1:length(groupsToPlotFst) 
                xx = vcat(first(windowed_pos_selected), windowed_pos_selected, last(windowed_pos_selected))
                FstRow = findfirst(pairwiseNamesFst .== groupsToPlotFst[j])
                yy = vcat(0, windowed_Fst_selected[FstRow, :], 0)
                poly!(axs[rowNum], Point2f.(xx, yy), color = groupColorsFst[j])
            end
        end
    end
    display(f)
...
#########
RCODE:
# plot Genome-wide plot of windowed Fst
...
#### Enter the chromosome (scaffold) lengths
