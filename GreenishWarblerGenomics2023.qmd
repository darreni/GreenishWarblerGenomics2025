...
### List the scaffolds to be imputed and included in PCA
...
function getWindowedFst(FstNumerator, FstDenominator, pos, windowSize)
    ...
    # note regarding NaN values: I checked and confirmed that wherever FstNumerator has NaN values,
    # so does FstDenominator.
    # My rollingMean function is designed to ignore NaN values, or return NaN if there are no real values.
end
...
NO LONGER NEEDED: Remove the first column of the genotype matrix (which was an initial row number):
```julia
...
group2 = "plumb"
groupsToCompare = "Fst_among" #"Fst_among"
Fst_cutoff = 0.6
missingFractionAllowed = 0.2  # only show SNPs with less than this fraction of missing data among individuals
...
#genotypes_region = genosOnly_with_missing[:, loci_selection]

# temp defs for stepping into function
# genotypes = genotypes_region
# pos = pos_region
...
# Now standardize the heterozygosity in each window by diving by the average:

meanPerWindow_windowedIndHet = sum(windowedIndHet, dims=1) ./ size(windowedIndHet, 1)
windowedIndHet_standardized = Array{Float64, 2}(undef, size(windowedIndHet, 1), size(windowedIndHet, 2))
for row in 1:size(windowedIndHet,1)
...
variancePerWindow_windowedIndHet = var(windowedIndHet_standardized; dims=1)

lines(windowedPos, vec(variancePerWindow_windowedIndHet))
...
```{julia}
scaffolds_to_plot = "gw" .* string.(vcat(1:15, 17:28))
push!(scaffolds_to_plot, "gw1A", "gw4A")  # add two other scaffolds
```

Making this inactive because takes long:
...
DON'T ACTUALLY USE THIS BLOCK BELOW TO MAKE A PCA, BECAUSE BASED ON IMPUTING THE WHOLE THING RATHER THAN CHROMOSOME BY CHROMOSOME:
...
```
Now make the whole-genome PCA (DON'T USE BECAUSE NOT PER-CHR IMPUTATION):
...
save("Figure_1b_from_Julia.svg", GW_wholeGenome_PCA.PCAfig)
```

#### PCA plot of the east side of the ring, for one chromosome (DON'T USE BECAUSE NOT PER-CHR IMPUTATION)
...
chr = "gwZ"
regionInfo = chooseChrRegion(pos_SNP_filtered_gwZ_SNPfiltered, chr; positionMin=1, positionMax=scaffold_lengths[chr]) # this gets the maximum position for the chromosome
