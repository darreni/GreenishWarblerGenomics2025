...
# create a vector that indicates the groups for this analysis:
clusterMembership = fill("none", length(PCAmodel.metadata.Fst_group))
for i in eachindex(clusterArray[1,:])
    clusterMembership[Bool.(clusterArray[:,i])] .= clusterNames[i]
end

# Calculate allele freqs and sample sizes
freqs, sampleSizes = getFreqsAndSampleSizes(geno_lowIndHetStan, cluster_membership, clusterNames)
...
PCAmodel.metadata.Fst_group = cluster_membership

plotInfo = plotGenotypeByIndividual(groupsToCompare, Fst_cutoff, missingFractionAllowed,
    regionInfo, pos_highHetVarRegion, Fst, pairwiseNamesFst,
    geno_lowIndHetStan, PCAmodel.metadata, freqs, clusterNames, clusterColors; plotTitle = "")
```
