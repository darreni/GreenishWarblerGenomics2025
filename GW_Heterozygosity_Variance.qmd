...
## Make a PCA based on all non-LHBRs (that is, outside of the identified Large HaploBlock Regions):
...
for i in eachrow(highHetVarRegions) 
...
for i in eachindex(chromosomes_to_include)
    ...
    @time imputed_genos = Impute.knn(genosOnly_nonLHBR_region_for_imputing; k=1, dims=:rows)
    ...
    println(string(regionText, ": Saved real and imputed genotypes for ", size(pos_SNP_filtered_nonLHBR_region, 1)," SNPs and ", size(genosOnly_nonLHBR_region_for_imputing, 1)," filtered individuals, in file $filename"))
end
```
...
# PCA of all individuals (IMPUTING ALL AT ONCE--SHOULD PROBABLY CHANGE TO CHR-BY-CHR):

```{julia}
genos_nonLHBR_imputed = Impute.svd(Matrix{Union{Missing, Float32}}(genosOnly_included_nonLHBR))

flipPC1 = true
flipPC2 = false

PCAmodelAll = plotPCA(genos_nonLHBR_imputed, ind_with_metadata_included, 
            groups_to_plot_PCA, group_colors_PCA; 
            sampleSet = "greenish warblers", regionText = "non-LHBR whole genome",
            flip1 = flipPC1, flip2 = flipPC2,
            lineOpacity = 0.7, fillOpacity = 0.6,
            symbolSize = 14, showTitle = true,
            xLabelText = string("PC1"), yLabelText = string("PC2"),
            showPlot = false)
...
freqs, sampleSizes = getFreqsAndSampleSizesBySexForZ(genos_highHetVarRegion, ind_with_metadata_included.sex, clusterMembership, clusterNames)
...
PCAmodel = plotPCA(genos_highHetVarRegion_imputed, ind_with_metadata_included, 
...
PCAmodelHapWest = plotPCA(genos_highHetVarRegion_imputed, ind_with_metadata_included_temp, 
            groups_to_plot_PCA_westside, group_colors_PCA_westside; 
...
PCAmodelHapEast = plotPCA(genos_highHetVarRegion_imputed, ind_with_metadata_included_temp, 
            groups_to_plot_PCA_eastside, group_colors_PCA_eastside; 
...
### At chr 15 high HetVar region, there are only 5 clear haplogroups (PC3 does not distinguish vir and lud). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### At chr 28 high HetVar region, there are only 5 clear haplogroups (yellow and orange are close). Vir and lud don't distinguish along PC3, as nitidus varies there. Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
## At chr 26 high HetVar region, there are only 5 clear haplogroups (one green is somewhat away). Vir and lud vary along PC3 but cannot be clearly grouped. Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### Do a PCA based on a same-size region elsewhere on gw26 (with low HetVar):
...
### At chr 1A high HetVar region, there are only 5 clear haplogroups (checked that vir and lud don't form distinct groups on PC3). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### At chr 2 high HetVar region, there are 5 clear haplogroups. Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### At chr 3 high HetVar region, there are only 4 clear haplogroups (vir and lud separated though on PC3, but not in a clean way that I can distinguish clearly). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### At chr 18 high HetVar region, there are 5 clear haplogroups (vir and lud separated though on PC3, though not clearly enough to indicate as different in summary plot). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
## Tried chr 12 but sort of a mess. The ludlowi samples fall in all clusters, even plumb!
...
### At chr 13 high HetVar region, there are 6 clear haplogroups (vir and lud separated cleanly on PC3, with one hetero between them). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### Do a PCA based on a same-size region elsewhere on gw13 (with low HetVar):
...
## Tried chr 14 but not very clear--seems recombination backcrosses in ludlowi
...
vir = (PCAmodelAll.metadata.PC1 .< -6.5) .&
            (0.5 .< PCAmodelAll.metadata.PC2 .< 4) .&
            (0 .< PCAmodelAll.metadata.PC3) .&
            indSelection_lowIndHetStan
nit = (-6 .< PCAmodelAll.metadata.PC1 .< -4.5) .&
        (0.5 .< PCAmodelAll.metadata.PC2 .< 1.5) .&
            indSelection_lowIndHetStan
lud = (PCAmodelAll.metadata.PC1 .< -6.5) .&
            (0.5 .< PCAmodelAll.metadata.PC2 .< 4) .&
            (PCAmodelAll.metadata.PC3 .< 0) .&
            indSelection_lowIndHetStan
...
─────┼─────────────────────────
   1 │ vir_nit       0.0230516
   2 │ vir_lud       0.0137261
   3 │ vir_troch     0.0345623
   4 │ vir_obs       0.0343802
   5 │ vir_plumb     0.0369935
   6 │ nit_lud       0.0200072
   7 │ nit_troch     0.0339931
   8 │ nit_obs       0.0337221
   9 │ nit_plumb     0.036556
  10 │ lud_troch     0.0314598
  11 │ lud_obs       0.0312961
  12 │ lud_plumb     0.0339024
  13 │ troch_obs     0.0201926
  14 │ troch_plumb   0.0240156
  15 │ obs_plumb     0.0195389 =#
...
vir_troch = (-5 .< PCAmodelAll.metadata.PC1 .< 0) .&
                (-4 .< PCAmodelAll.metadata.PC2 .< -0.7) .&
                (0.5 .< PCAmodelAll.metadata.PC3 .< 2)
                 .!indSelection_lowIndHetStan
lud_troch = (-5 .< PCAmodelAll.metadata.PC1 .< 0) .&
                (-4 .< PCAmodelAll.metadata.PC2 .< -0.7) .&
                (-2.5 .< PCAmodelAll.metadata.PC3 .< 0.5)
                 .!indSelection_lowIndHetStan
lud_obs = (-3 .< PCAmodelAll.metadata.PC1 .< -1) .&
...
clusterArray = [vir nit lud vir_troch lud_troch troch lud_obs obs troch_plumb plumb vir_plumb]
...
### At chr 19 high HetVar region, there are 4 clear homozygous haplogroups (vir and lud separated though on PC3, but not clearly enough to show in summary plot). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### At chr 4A high HetVar region, there are 4 clear haplogroups (no discernment of lud or obs on PC3, which is driven by nitidus). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
### Do a PCA based on a same-size region elsewhere on gw3 (with low HetVar):
...
rightLocus = scaffold_lengths["gw3"] - 1_000_000
leftLocus = rightLocus - lengthHighHetVarRegion
...
### At chr 20 high HetVar region, there are 6 clear haplogroups (with vir and lud clearly different on PC3). Divide samples into those groups, based on PCA scores, and then calculate pi within and pi between
...
haploblockRegions = ["gw1A", "gw3", "gw3", "gw13", "gw15", "gw17", "gw18", "gw19", "gw20","gw26", "gw28", "gwZ"]
...
#########################
EDIT / REVISE THIS PART BELOW TO PROPERLY INCORPORATE THE Z CHROM FILTERING
...
#plot(windowedFstValues, windowed_HetVariance_all)
...
# hidedecorations!(ax, label = false, ticklabels = false, ticks = false) # hide background lattice
plot!(ax, windowedFstValues, windowed_HetVariance_all,
