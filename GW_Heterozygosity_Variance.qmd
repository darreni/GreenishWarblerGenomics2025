...
## switch plot order to ring_km, and set nitidus (set to -2500 km) and the Siberian hybrid (set to 5000 km) plot order:
...
sum_highHetVarRegions = sum(highHetVarRegions.regionEnd .- highHetVarRegions.regionStart)
...
percentGenomeHighViSHet = 100 * sum_highHetVarRegions / sum_scaffold_lengths
...
fig2 = plotGenomeHetVariance(scaffolds_to_plot, 
                        windowed_HetVariance_all,
                        windowed_pos_all;
                        fillColor = "purple",
                        lineTransparency = 0.8,
...
Did this on 1Nov2024 and saved files, so inactivated this cell for now (takes about 0.5 to 6 min per scaffold):
...
for i in eachindex(chromosomes_to_include)
    ...
    @time imputed_genos = Impute.svd(genosOnly_nonLHBR_region_for_imputing)
...
### Do a PCA based on a same-size region elsewhere on gw4A (with low HetVar):
...
for i in 1:length(scaffolds_to_plot)
    ...
    highHetVarRegions_thisScaffold = highHetVarRegions[highHetVarRegions.regionChrom .== chr, :]
    ...
    plotInfo = plotGenotypeByIndividualWithFst(groupsToCompare, Fst_cutoff, 
        ...
        highlightRegionStarts = highHetVarRegions_thisScaffold.regionStart,
        highlightRegionEnds = highHetVarRegions_thisScaffold.regionEnd,
...
# code taken from GenomicDiversity.plotPCA :
