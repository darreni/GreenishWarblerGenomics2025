...
```{julia}
genosOnly_with_missing = Matrix{Union{Missing, Int32}}(genosOnly)
...
```{julia}
genosOnly_included, ind_with_metadata_included = limitIndsToPlot(plotGroups, numIndsToPlot, genosOnly_with_missing, ind_with_metadata_indFiltered);
```
...
Looks good. Now impute and do the PCA:
...
if do_imputing
    ...
    # took 102 sec
    jldsave(filename; imputed_genos, ind_with_metadata_indFiltered_sex, pos_SNP_filtered_gwZ_SNPfiltered)
    ...
    ind_with_metadata_indFiltered_sex_chrZcleaned = load(filename, "ind_with_metadata_indFiltered_sex")
    pos_SNP_filtered_chZcleaned = load(filename, "pos_SNP_filtered_gwZ_SNPfiltered")
    println(string("Loaded ",filename))
