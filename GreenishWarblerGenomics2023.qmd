...
function rollingMean(inputVector, windowSize)
    ...
    for i in 1:numWindows
        ...
        values = inputVector[start : (start + windowSize - 1)]
        rollingMeans[i] = sum(values) / length(values)
    end
    return rollingMeans
end


demoVector = collect(1:2:10000)
rollingMean(demoVector, 1000)

selection = (pos.chrom .== chr)

pos = pos_SNP_filtered

function getWindowedFst(FstNumerator, FstDenominator, pos, windowSize; stepSize = windowSize)
    rollingMeanPos = rollingMean(pos)
