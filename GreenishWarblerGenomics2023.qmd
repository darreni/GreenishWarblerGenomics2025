...
```{julia}
genotypes = geno_ind_SNP_ind_filtered[:, Not(1)]
```

For missing genotypes, change our code of `-1` to `missing`:

```{julia}
genos_with_missing = Matrix{Union{Missing, Int32}}(genotypes)
...
genotypes_gwZ = genotypes[:,loci_selection]
...
I don't think number missing is very helpful--probably similar in males and females.

Maybe will try this: see which SNPs are associated with PC2 of the Z chromosome PCA, and see which show high Fst between the high heterozygosity and low heterozygosity groups. If similar, then remove the SNPs with high 
